...
PROMO_CREATE_PROMPT = (
    "Введите промокод одной строкой:\n"
    "<code>КОД [uses|time|users] МАКС_ИСП/ЧАСОВ/USERNAME НАГРАДА1:КОЛ НАГРАДА2:КОЛ ...</code>\n\n"
    "Типы: <code>uses</code> — лимит активаций, <code>time</code> — часов действия, "
    "<code>users</code> — список username через запятую (без пробелов).\n"
    "Награды (можно несколько, через пробел): <code>tickets</code> 🎫, <code>coins</code> 💎, "
    "<code>dust</code> ✨. Количество может быть отрицательным — спишет (не уйдёт ниже 0).\n\n"
    "Примеры:\n"
    "<code>WELCOME uses 50 coins:1000 tickets:5</code>\n"
    "  → 50 использований, коины + тикеты\n"
