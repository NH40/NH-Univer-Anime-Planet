...
_REGEN_AND_SPEND_SQL = text(
    ...
    WITH regen AS (
        SELECT
            ...
            CASE WHEN subscription_until > now() THEN :interval_sub ELSE :interval_normal END AS effective_interval,
            CASE
                ...
                ELSE LEAST(
                    ...
                    tickets_count + FLOOR(
                        ...
                        (CASE WHEN subscription_until > now() THEN :interval_sub ELSE :interval_normal END)
                    )::int
                )
            END AS new_count
        FROM users
